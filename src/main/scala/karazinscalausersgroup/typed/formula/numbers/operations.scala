...
object operations {
  ...
  object Σ {
    def withFoldable[E <: Expression[Number]](foldable2: Foldable[Number, Foldable[Number, Value[Number]]])
                                             (implicit
                                          defaults: List[Value[Number]],
                                          builder: ExpressionBuilder[Number, E]): Σ[E] =
      new Σ(foldable2, defaults, builder)

    def withFoldableAndDefaults[E <: Expression[Number]](foldable2: Foldable[Number, Foldable[Number, Value[Number]]],
                                                         defaults: List[Value[Number]])
                                                        (implicit
                                                     builder: ExpressionBuilder[Number, E]): Σ[E] =
      new Σ(foldable2, defaults, builder)
